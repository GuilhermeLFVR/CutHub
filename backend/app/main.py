...
def seed_admin(db: Session = Depends(get_db)):
    return crud.ensure_default_admin(db, hash_password("admin123"))
...
def identify_client_by_face(
    ...
    """Reconhecimento facial assistido para demonstração realista.

    Estratégia usada no CutHub:
    1. Prioriza clientes com atendimento marcado para hoje, porque na barbearia a câmera
       normalmente identifica alguém que acabou de chegar para um horário.
    2. Usa OpenCV quando disponível para comparar o rosto capturado com os rostos cadastrados.
    3. Se a confiança ficar baixa, retorna candidatos para o barbeiro confirmar.
    4. Se OpenCV não estiver instalado, cai para o candidato da agenda do dia, mantendo a
       apresentação estável sem fingir uma precisão que o ambiente não consegue garantir.
    """
    raw_data = payload.image_data.strip()
    match = re.match(r"^data:image/(png|jpeg|jpg);base64,(.+)$", raw_data)
    if not match:
    ...
    # Em uso real, a câmera na recepção/tablet deve olhar primeiro para quem tem horário hoje.
    ...
    if ranked:
        ...
        print("DEEPFACE: nenhum rosto reconhecido")
        ranked = []
    ...
    low_confidence = best.get("source") != "opencv" or score < 0.58

    if best.get("source") == "agenda":
        message = "OpenCV não está disponível ou não encontrou rosto com confiança. Mostrando melhor candidato pela agenda do dia."
    elif low_confidence:
